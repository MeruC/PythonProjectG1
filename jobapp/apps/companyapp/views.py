...
from ..profileapp.views import get_user_education, get_user_work_experience
...
# -------- To view the applicant's resume --------

def generateApplicantResume(request, applicant_id):
    applicant = get_object_or_404(jobApplicant, id=applicant_id)
    
    pdf = FPDF('P', 'mm', 'A4')
    
    userN = "_".join(applicant.user.last_name.split(" ")) + "_" + "_".join(applicant.user.first_name.split(" ")) + "_Resume"
    
    pdf.set_title(applicant.user.last_name + ", " + applicant.user.first_name + " Resume")
    ...
    pdf.image(applicant.user.profile_img, 170, 8, 25)
    pdf.set_font("helvetica", "", 24)
    pdf.set_text_color(230,230,230)
    pdf.cell(0, 4, "", border=False, ln=1, align='L')
    pdf.cell(0, 7, applicant.user.first_name + " " + applicant.user.last_name, border=False, ln=1, align='L')
    pdf.set_text_color(180,180,180)
    pdf.set_font("helvetica", "", 14)
    pdf.cell(0, 7, applicant.user.email, ln=1, align='L')
    pdf.set_font("helvetica", "", 12)
    pdf.cell(0, 4, "+63" + applicant.user.contact_number, ln=1, align='L')
    pdf.ln(20)
    ...
    pdf.multi_cell(0, 5, "\t\t\t\t\t\t\t\t\t\t" + applicant.user.profile_summary, align="J")
    ...
    work_experiences = get_user_work_experience(applicant)
    last_work = work_experiences.last()
    ...
    education = get_user_education(applicant)
    last_education = education.last()
    ...
    ##Skills
    line_width = pdf.get_string_width("\t\t\t\t\t\t\t\t\t\t" + ', '.join(i for i in applicant.user.skills.split(',')))
    n = line_width / 155
    if (n > 3):
        pdf.add_page()
    pdf.cell(0, 8, "", ln=True)
    ...
    pdf.set_font('helvetica', '', 14)
    pdf.set_text_color(0, 0, 0)
    pdf.multi_cell(0, 5, "\t\t\t\t\t\t\t\t\t\t" + ', '.join(i for i in applicant.user.skills.split(',')), align="J")
